...
{
    const QString config_dir = seafApplet->configurator()->ccnetDir();
    const QString wingufile_dir = seafApplet->configurator()->wingufileDir();
    ...
    QStringList args;
    args << "-c" << config_dir << "-d" << wingufile_dir << "-w" << worktree_dir;
...
{
    qDebug("wingufile daemon is now running");
    emit daemonStarted();
}
...
void DaemonManager::onSeafDaemonExited()
{
    ...
    seafApplet->errorAndExit(tr("wingufile daemon has exited abnormally"));
}

void DaemonManager::stopAll()
{
    qDebug("[Daemon Mgr] stopping ccnet/wingufile daemon");
    if (seaf_daemon_)
